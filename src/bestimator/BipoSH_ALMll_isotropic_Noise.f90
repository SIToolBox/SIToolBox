...
subroutine isotropicnoise(llmax,LMAX,clbllMAX,clbLMAX,nside,samplenumber,noisevariance,fileinput,fileclebsch,outfilepath)
   ...
   integer :: samplenumber,samp1,ll,lloopmax
   ...
   do i=0,llmax
      ...
      write(144,*)cl(i,1)        !
      Nl(i) = 0.0004*(noisevariance/10)**2             ! Initiallize noise matrix
      write(*,*)Nl(i)
   end do                        !  
   close(unit=144) 
   ...
   do samp1=0,samplenumber !5000
     ...
     write(*,*)'Sample Number :',samplenumber,samp1,repl
...
!$omp end parallel do
  ...
  !!
  !!     Important. Please don't remove or modify this part
  !!
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       do l1=llmin,10
         ...
         if((ALMll(0,0,l1,l1)*(-1)**l1) > (IALMll(l1)*((-1)**l1)*(1.0+10.0/sqrt(2.0*l1+1)))) then
       ...
       do l1=40,llmax
         if(abs(ALMll(0,0,l1,l1)) .lt. 0.000001) then
           ALMll(0,0,l1,l1) = 0.000001*ALMll(0,0,l1,l1)/abs(ALMll(0,0,l1,l1))
         !!end if
         write(*,*)'Here',l1
         endif
       end do
...
subroutine initM(Malmr,Malmi,llmax,cl,Nl)
   ...
   do i=0,(llmax+1)*(llmax+2)/2-1
      call n2lm(i,l,m)
      if((abs(cl(l,1)-Nl(l))).gt.1.0d-6) then 
        Malmr(i) = 1.0/(abs(cl(l,1)-Nl(l)))+1.0/Nl(l)
        Malmi(i) = 1.0/(abs(cl(l,1)-Nl(l)))+1.0/Nl(l)
      else
        Malmr(i) = 1.0/(1.0d-6)+1.0/Nl(l)
        Malmi(i) = 1.0/(1.0d-6)+1.0/Nl(l)
      end if
   end do
   return
end subroutine
...
!$omp localimmax, jm, j, SMat, SMati )
     ...
     do i=0,Nmax 
       ...
       Sum1i = Sum1i/(ALMll(0,0,il,il)*Clebs(recno)*int((-1)**im)) 
