...
noisefileinput,clebschpath,maskpath,chainpath,maskparam,isoparam)
   ...
   real(dp) :: timestart,timeend,noipix
       ...
       do i=0,MyLMax
         do j=0,i
           do kk=0,i
              ...
              l1 = l1+1
              write(8154+l1,"(1025(E20.5))")Clo(:,1)
              write(9168+l1,"(1025(E20.5))")Clo(:,2)
...
!write(*,*)'I am now here and OK'
!     write(*,*)'Sample Number :', samplenumber,samp1
...
!        dfalm(1,l,m) = pixwin(l)*cmplx(Qr(i), Qi(i))
     end do
!$omp end parallel do
...
!$omp   private ( i, l, m )
       ...
       do i=0,lloopmax
         call n2lm(i,l,m)
         if(m.ne.0) then 
           Palmrdot(i)  = 2.0*dfQr(i) -2.0*SMapr(i) !+ 2.0*dfQr(i) !(Dr(i) - Qr(i))/Nl(l) 
           Palmidot(i)  = 2.0*dfQi(i) - 2.0*SMapi(i) !+ 2.0*dfQi(i) !(Di(i) - Qi(i))/Nl(l) 

         else 
           Palmrdot(i)  =  1.0*dfQr(i) - 1.0*SMapr(i) !+ 1.0*dfQr(i) !(Dr(i) - Qr(i))/Nl(l)
           Palmidot(i)  = 0.0
         end if
       end do
...
!$omp end parallel do
  ...
  !! 
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!      write(*,*)"I can reach here"
       do l1=llmin,30
         if((ALMll(0,0,l1,l1)*(-1)**l1) .lt. 1) then
           ALMll(0,0,l1,l1) = 1*ALMll(0,0,l1,l1)/abs(ALMll(0,0,l1,l1))
         end if

         if(abs(ALMll(0,0,l1,l1)*(-1)**l1) > abs(IALMll(l1)*((-1)**l1)*(1.0+8.0/sqrt(2.0*l1+1)))) then
           ALMll(0,0,l1,l1) = IALMll(l1) 
           write(*,*)l1,'. Getting stuck here' 
         end if
       end do


       do l1=llmin,llmax
         if(abs(ALMll(0,0,l1,l1)) .lt. 0.0001) then
           ALMll(0,0,l1,l1) = 0.0001*ALMll(0,0,l1,l1)/abs(ALMll(0,0,l1,l1))
           write(*,*)l1,'. Getting stuck'
         end if
       end do

     end do

     if(samp1.eq.80) then  !! We consider 80 as burnin End
        do l1=llmin,llmax-LMAX
!           write(*,*)'testimg the mechanism : ',l1
           IALMll(l1) = ALMll(0,0,l1,l1)
        end do
     end if
     write(*,*)'I am here now' 
     if(samp1.gt.80) then
        do l1=llmin,llmax
           if(abs(ALMll(0,0,l1,l1)*(-1)**l1) > abs(IALMll(l1)*((-1)**l1)*(1.0+6.0/sqrt(2.0*l1+1)))) then
                ALMll(0,0,l1,l1) = IALMll(l1)
                write(*,*)l1,'. Getting stuck here. Fixed by replacing the value. ERROR INDEX : 432 '
           end if
        end do
     end if
      ...
      do i=0,MyLMax
        do j=0,i
          do kk=0,i
             ...
             l1 = l1+1
             write(8154+l1,"(9999(E20.5))")Clo(:,1)
             write(9168+l1,"(9999(E20.5))")Clo(:,2)
          end do
        end do
      end do

   call cpu_time(timeend)
   write(*,*)'Sample Number :', samp1,'(',samplenumber,')',repl,'Time = ',timeend-timestart

   if(modulo(samplenumber, 100) .eq. 0)then
      k=0
      do i = 0,llmax
        do j = 0,i
      ...
      write(testfile,"(I5.5)")sampl
      testfile = 'Mymap'//trim(testfile)//'.d'
      open(unit=1468,file=testfile)
      do i=0,Npix-1
         write(1468,*)Map2(i,1)
      end do
      close(1468)
